...
public class BungeeUserListener extends UserListener implements Listener {
	...
	public void onServerConnect(ServerSwitchEvent event) {
		...
		ProxyPlayer player = new BungeePlayer((PortfelBungeeImpl) this.getPlugin(), event.getPlayer());
		if (srv.isPresent()) {
			this.onConnect(player, new BungeeServerConnection((PortfelBungeeImpl) this.getPlugin(), srv.get()));
		}
	}
