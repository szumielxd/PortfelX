...
public class MinorEcoTakeCommand<C> extends SimpleCommand<C> {
	...
	@Getter private final @NotNull LangKey description = ProxyLangKey.COMMAND_USER_MINORECO_TAKE_DESCRIPTION;
	
	
	public MinorEcoTakeCommand(@NotNull Portfel<C> plugin, @NotNull ParentLikeCommand<C> parent) {
	...
	public void onCommand(@NotNull CommonSender<C> sender, @NotNull ParsedCommandContext parsedContext) {
		this.parseArguments(sender, parsedContext).ifPresent(parsed -> {
			...
			if (user.getBalance() < amount) {
				ProxyLangKey.COMMAND_USER_MINORECO_TAKE_SMALLER.draft()
						.sendPrefixed(sender);
				return;
			}
			user.takeMinorBalance(amount);
			ProxyLangKey.COMMAND_USER_MINORECO_TAKE_SUCCESS
					.draft(
							user.getName(),
